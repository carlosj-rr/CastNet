...
def main_experiment():
    ...
    #Ex_1 d_split,abc_split = randsplit(founder_pop,100) # split the founder population into the lineage that will go on to make the D tip, and the one that is ancestral to all other remaining lineages
    #Ex_1 D_tip = branch_evol(d_split,15000,branch_id="TipD") # simulate the evolution of the D lineage over 15K generations
    #Ex_1 abc_ancestor = branch_evol(abc_split,1000,branch_id="ABC_anc") # simulate the evolution of the ancestor to A,B and C for 1K generations
    #Ex_1 bc_split, c_split = randsplit(abc_ancestor,100) # split the resulting ancestor into two populations, the one that will go on to make the C tip, and the one that will be ancestral to A and B
    #Ex_1 C_tip = branch_evol(c_split,5000,branch_id="TipC") # simulate the evolution of lineage C for 5K generations
    #Ex_1 ab_ancestor = branch_evol(bc_split,4000,branch_id="AB_anc") # simulate the evolution of the ancestor to A and B for 4K generations
    #Ex_1 a_split,b_split = randsplit(ab_ancestor,100) # split the ancestor to A and B into the ancestor of A and the ancestor of B.
    #Ex_1 A_tip = branch_evol(a_split,500,branch_id="TipA") # simulate the evolution of the A tip for 500 generations
    #Ex_1 B_tip = branch_evol(b_split,10000,branch_id="TipB") # simulate the evolution of the B tip for 10K generations
    heterotachy_step1 = branch_evol(founder_pop,1000,"SlowBit")
    pf.seq_mutation_rate = pf.seq_mutation_rate*2
    pf.pop_size = 100
    pf.reporting_freq = 10
    heterotachy_step2 = branch_evol(heterotachy_step1,1000,"CrazyBit")
        
    return([founder_pop,heterotachy_step2]) #Ex 1: [founder,D_tip,C_tip,B_tip,A_tip]

if __name__ == "__main__":
    result = main_experiment()
    store(result)
    tip_names=["founder","heterotachy"] #Ex 1: ["founder","D","C","B","A"]
