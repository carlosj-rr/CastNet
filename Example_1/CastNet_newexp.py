        ...
        for gen in tqdm(range(ngens),desc=f"{branch_id} branch evolution progress:", ascii=False,ncols=100):
            if parent_pop.size > 0:
                ...
                if type(survivors) == bool:
                    ...
                    store(parent_pop,filename) # NORMAL EXIT
                    return
...
def main_experiment():
    ...
    d_split,abc_split = randsplit(founder_pop,100) # split the founder population into the lineage that will go on to make the D tip, and the one that is ancestral to all other remaining lineages
    D_tip = branch_evol(d_split,15000,branch_id="TipD") # simulate the evolution of the D lineage over 15K generations
    abc_ancestor = branch_evol(abc_split,1000,branch_id="ABC_anc") # simulate the evolution of the ancestor to A,B and C for 1K generations
    bc_split, c_split = randsplit(abc_ancestor,100) # split the resulting ancestor into two populations, the one that will go on to make the C tip, and the one that will be ancestral to A and B
    C_tip = branch_evol(c_split,5000,branch_id="TipC") # simulate the evolution of lineage C for 5K generations
    ab_ancestor = branch_evol(bc_split,4000,branch_id="AB_anc") # simulate the evolution of the ancestor to A and B for 4K generations
    a_split,b_split = randsplit(ab_ancestor,100) # split the ancestor to A and B into the ancestor of A and the ancestor of B.
    A_tip = branch_evol(a_split,500,branch_id="TipA") # simulate the evolution of the A tip for 500 generations
    B_tip = branch_evol(b_split,10000,branch_id="TipB") # simulate the evolution of the B tip for 10K generations
    ##crazy_branch_step1 = branch_evol(founder_pop,1000,"SlowBit")
    ##pf.seq_mutation_rate = pf.seq_mutation_rate*2
    ##pf.pop_size = 10
    ##pf.reporting_freq = 10
    ##crazy_branch_step2 = branch_evol(crazy_branch_step1,1000,"CrazyBit")
    #d_split,abc_split = randsplit(founder_pop,100)
    #D_tip = branch_evol(d_split,15000,branch_id="TipD")
    #abc_ancestor = branch_evol(abc_split,1000,branch_id="ABC_anc")
    #bc_split, c_split = randsplit(abc_ancestor,100)
    #C_tip = branch_evol(c_split,5000,branch_id="TipC")
    #ab_ancestor = branch_evol(bc_split,4000,branch_id="AB_anc")
    #a_split,b_split = randsplit(ab_ancestor,100)
    #A_tip = branch_evol(a_split,500,branch_id="TipA")
    #B_tip = branch_evol(b_split,10000,branch_id="TipB")
    
    return([founder_pop,D_tip,C_tip,B_tip,A_tip])

if __name__ == "__main__":
    result = main_experiment()
    store(result)
    tip_names=["founder","Lin D","Lin C","Lin B","Lin A"]
