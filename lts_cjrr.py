...
def main(X, true_W, y):
    W_guess = find_linear_transform(
        X, y
    )  # <--- this function should not take the correct W in any case, LoL
...
decayed_v0 = (lambda x, l: x * np.exp(-l))(
    v0, decay_lambdas
...
pre_thresholds = (
    exp_change + decayed_v0
)  # add the decayed amounts to the regulatory effects
thresh_rectifier = (pre_thresholds > exp_thre).astype(
    int
...
if __name__ == "__main__":
    ...
    true_W = np.array(
        [  # This is a matrix I'm trying to learn, in this case I know the correct one since I defined
            [
                4.0,
                7.0,
                3.0,
            ],  # it here. Normally, I don't have this matrix and I'm trying to infer it with this method
            [1.0, 8.0, -1.0],
            [-5.0, -6.0, 11.0],
        ]
    )
